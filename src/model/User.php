...
class User {
    ...
    function inGroup($group) {
        return array_key_exists($group,$this->groups) && $this->groups[$group];
    }
    ...
    function authenticate($user,$pass) {
        ...
        if(strlen($result[0]['password']) !=0){
            $this->setPassword($result[0]['id'], $result[0]['password']);
    ...
    function setPassword($id, $password){
        ...
        $sql = "UPDATE user
                ...
                WHERE id =:id";
        $prepared = $db->prepare($sql, array($db->ATTR_CURSOR => $db->CURSOR_FWDONLY));
        $prepared->execute(array(':id' => $id, ':password' => "", ':hash'=>$hash,':nonce'=>$nonce));
    }
    ...
    function register($username, $password){
        ...
        $prepared = $db->prepare($sql, array($db->ATTR_CURSOR => $db->CURSOR_FWDONLY));
        $status = $prepared->execute(array(':username' => $username, ':password' => "", ':hash' => $hash, ':nonce'=> $nonce));
